...
function tezaurus() {
	...
	jQuery.ajax({
 			...
 			success: function(data) {
 				jQuery("#result").html('');
 				if (data == "no_sinonim") {
 					jQuery("#result").append('<span class="no_sinonim">ДӀаяздинчу дешан синоним ца карайо!</span>').slideDown(200);
 				}
 					...
 					// if (mass.length > 1) var sinLength = "синонимаш";
 					...
 					for (var j = 0; j < mass.length; j++) {
 						...
 						for (var i = 0; i < mass[j].length; i++) {
	 						...
	 						if (sinonimy != null) {
	 							slovo = '<a href=javascript:void(0) id="slovo'+i+'" title="ХӀокху дешан синонимашка хьажа дош тӀетаӀаде.">'+slovo+'</a>';
	 						}
